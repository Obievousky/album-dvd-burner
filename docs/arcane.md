# Deploying on Arcane (remote Ubuntu VM)

Deploy `docker-compose.arcane.yml` on your Ubuntu VM, then open **http://\<vm-ip\>:8080**.
...
2. Arcane → Stacks → `docker-compose.arcane.yml`
3. Set env: `POSTGRES_PASSWORD`, `DATA_ROOT=/opt/album-dvd-burner`
